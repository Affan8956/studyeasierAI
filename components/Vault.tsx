  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar pb-24 md:pb-8">
      <div className="max-w-6xl mx-auto pt-10 md:pt-0">
        ...
        {/* Tab Switcher */}
        <div className="flex bg-[#151515] p-1.5 rounded-2xl mb-8 border border-slate-800 w-full max-w-lg">
           <button 
           ...
           <button 
             ...
             className={`flex-1 py-2.5 md:py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${vaultTab === 'requests' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
           >
          ...
          <div className="animate-fadeIn">
            {pendingRequests.length === 0 ? (
               <div className="py-24 text-center">
                  ...
                  <p className="text-slate-500 font-medium">No pending sharing requests.</p>
               </div>
            ) : (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {pendingRequests.map(req => (
                    <div key={req.request_id} className="bg-[#121212] border border-slate-800 p-6 rounded-3xl flex flex-col gap-4">
                       <div className="flex items-start justify-between">
                          <div>
                             ...
                             <p className="text-slate-400 text-sm">{req.asset_title}</p>
                          </div>
                          <div className="text-slate-600 text-[10px] font-bold">{new Date(req.created_at).toLocaleDateString()}</div>
                       </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {filteredAssets.length === 0 && (
                <div className="col-span-full py-24 text-center">
                  ...
                  <p className="text-slate-500 font-medium">No assets found matching criteria.</p>
                </div>
              )}
            </div>
          </>
           ...
           <div className="bg-[#0d0d0d] border border-slate-800 rounded-[2.5rem] p-6 md:p-10 max-w-md w-full shadow-2xl relative overflow-hidden">
              ...
              {!selectedUser ? (
                <div className="space-y-4">
                   <div className="relative">
                      <input 
                        ...
                        className="w-full bg-[#151515] border border-slate-800 rounded-2xl p-4 pl-10 text-slate-200 outline-none focus:border-indigo-500 text-sm"
                        autoFocus
                      />
                      <i className="fas fa-search absolute left-4 top-4 text-slate-500"></i>
                   ...
                   <div className="max-h-48 overflow-y-auto custom-scrollbar space-y-2">
                      {userSearchResults.map(u => (
                         <div 
                           ...
                           className="flex items-center gap-3 p-3 bg-[#151515] rounded-xl hover:bg-slate-800 cursor-pointer border border-transparent hover:border-indigo-500/30 transition-all"
                         >
                      ...
                      {userSearchQuery.length >= 3 && userSearchResults.length === 0 && shareStatus !== 'searching' && (
                         <p className="text-center text-xs text-slate-600 py-2">No users found.</p>
                      )}
                   </div>
                </div>
              ) : (
                <div className="bg-indigo-600/10 border border-indigo-500/30 p-4 rounded-2xl flex items-center justify-between mb-6">
                   ...
                   <button onClick={() => setSelectedUser(null)} className="text-slate-400 hover:text-white"><i className="fas fa-times"></i></button>
                </div>
              )}
