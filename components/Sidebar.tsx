...
const Sidebar: React.FC<SidebarProps> = ({ view, setView, chats, activeChatId, onSelectChat, onNewChat, onNewTutorChat, onDeleteChat, user, onLogout }) => {
  ...
  return (
    <div className="w-80 bg-[#0d0d0d] border-r border-slate-900 flex flex-col h-full z-50 flex-shrink-0">
        ...
        <nav className="space-y-1 mb-8">
          ...
          <div className="flex gap-1">
             <button 
               ...
               className={`flex-1 flex items-center gap-4 px-4 py-3 rounded-xl font-bold transition-all text-sm ${view === 'tutor' ? 'bg-indigo-500 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
             >
             ...
             {onNewTutorChat && (
               <button 
                 ...
                 className="w-12 flex items-center justify-center rounded-xl font-bold transition-all text-sm bg-slate-800 text-indigo-400 hover:bg-indigo-500 hover:text-white"
      ...
      <div className="p-6 bg-[#0a0a0a] border-t border-slate-900 mt-auto">
         ...
         <div className="text-[9px] text-slate-700 font-bold uppercase tracking-widest text-center">StudyEasierAI v3.4</div>
      </div>
    </div>
  );
};
