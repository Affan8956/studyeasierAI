    ...
    <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
      <div className="max-w-6xl mx-auto">
        <header className="mb-12">
          ...
          <h1 className="text-6xl font-black mb-4 tracking-tight leading-none text-text-main">
            Welcome, <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-violet-400">{firstName}</span>
          </h1>
          <p className="text-text-muted text-lg font-medium">Your intelligence modules are active and synchronized.</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
          <div 
            ...
            className="group p-10 bg-sidebar border border-border rounded-[2.5rem] hover:border-indigo-500/50 hover:bg-indigo-600/5 transition-all cursor-pointer shadow-2xl relative overflow-hidden"
          >
            ...
            <h3 className="text-2xl font-black mb-3 text-text-main">Smart Chat</h3>
            <p className="text-text-muted text-sm leading-relaxed font-medium">Initiate a deep reasoning session with Gemini 3 Pro intelligence.</p>
          ...
          <div 
            ...
            className="group p-10 bg-sidebar border border-border rounded-[2.5rem] hover:border-emerald-500/50 hover:bg-emerald-600/5 transition-all cursor-pointer shadow-2xl relative overflow-hidden"
          >
            ...
            <h3 className="text-2xl font-black mb-3 text-text-main">Knowledge Lab</h3>
            <p className="text-text-muted text-sm leading-relaxed font-medium">Extract structured mastery from lecture recordings and PDFs.</p>
          ...
          <div 
            ...
            className="group p-10 bg-sidebar border border-border rounded-[2.5rem] hover:border-amber-500/50 hover:bg-amber-600/5 transition-all cursor-pointer shadow-2xl relative overflow-hidden"
          >
            ...
            <h3 className="text-2xl font-black mb-3 text-text-main">Private Vault</h3>
            <p className="text-text-muted text-sm leading-relaxed font-medium">Access your global knowledge base and persistent assets.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
           <section>
              <div className="flex items-center justify-between mb-8 px-2">
                 <h4 className="text-[11px] font-black text-text-muted uppercase tracking-[0.3em]">Recent Discussions</h4>
                 <div className="h-[1px] flex-1 mx-6 bg-border"></div>
              </div>
              <div className="space-y-4">
                 {chats.length > 0 ? chats.slice(0, 4).map(chat => (
                   <div 
                     ...
                     className="p-5 bg-sidebar border border-border rounded-2xl flex items-center justify-between hover:border-indigo-500/30 transition-all group cursor-pointer hover:bg-surface"
                   >
                      <div className="flex items-center gap-5">
                        ...
                        <span className="font-bold text-sm truncate max-w-[200px] text-text-main">{chat.title}</span>
                      </div>
                      <span className="text-[9px] text-text-muted uppercase font-black tracking-widest">{new Date(chat.updatedAt).toLocaleDateString()}</span>
                   </div>
                 )) : (
           ...
           <section>
              <div className="flex items-center justify-between mb-8 px-2">
                 <h4 className="text-[11px] font-black text-text-muted uppercase tracking-[0.3em]">Intelligence Assets</h4>
                 <div className="h-[1px] flex-1 mx-6 bg-border"></div>
              </div>
              <div className="space-y-4">
                 {assets.length > 0 ? assets.slice(0, 4).map(asset => {
                   ...
                   return (
                     <div 
                       ...
                       className="p-5 bg-sidebar border border-border rounded-2xl flex items-center justify-between hover:border-emerald-500/30 transition-all cursor-pointer hover:bg-surface"
                     >
                        <div className="flex items-center gap-5">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${style}`}>
                             <i className={`fas text-xs ${icon}`}></i>
                          </div>
                          <span className="font-bold text-sm truncate max-w-[200px] text-text-main">{asset.title}</span>
                        </div>
                        <span className="text-[9px] text-text-muted uppercase font-black tracking-widest">{asset.type}</span>
                     </div>
                   );
                 }) : (
                   <div className="py-12 text-center border-2 border-dashed border-border rounded-[2rem]">
